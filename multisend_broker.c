...
void *send_message_to_subscribers_thread(void *args)
{
    char *topic = ((void **)args)[0];
    char *message = ((void **)args)[1];
    int start = *((int *)((void **)args)[2]);
    int end = *((int *)((void **)args)[3]);
...
void *handle_publisher(void *arg)
{
    ...
    if (subscriber_count > 0)
    {
        ...
        int subscribers_per_thread = subscriber_count / num_threads;
        int remainder = subscriber_count % num_threads;

        for (int i = 0; i < num_threads; i++)
        {
            int start = i * subscribers_per_thread;
            int end = (i + 1) * subscribers_per_thread;
            ...
            void *args[4] = {topic, message, &start, &end};

            pthread_create(&threads[i], NULL, send_message_to_subscribers_thread, (void *)args);
        }
